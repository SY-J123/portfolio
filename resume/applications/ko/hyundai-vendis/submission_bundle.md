...
### Natris / LULU.AI
Product Manager | 2024.04 - Present (2년)

- B2C 모바일 웹 포커 게임 기능 기획 및 UX 설계
- 기능 목적, 완료 기준, 예외 흐름을 포함한 요구사항 정의
- 입출력 데이터 기반 API 기능 정의
- 개발 구현 관점(props·상태)을 반영한 UI 기능 정의 및 UX 가이드라인 작성
- 바이브 코딩 기반 프로토타입 제작
- AX 협업 구조 개선 주도
> 성과: 검토 리드타임 1주 -> 3일 단축, 조직 정식 프로세스로 채택

### NSUSLAB Korea
Operations | 2023.08 - 2024.04 (9개월)

- GGPoker 토너먼트 운영 모니터링 및 지표 점검
- 해외 지사·파트너와 이벤트 스케줄, 포맷, 운영 요청 사항 조율 (영어)
- 참여율 데이터 기반 토너먼트 스케줄 재설계

### Me2on
Operations | 2022.08 - 2023.08 (1년 1개월)
...
저의 강점은 개선이 필요한 부분을 발견하면, 직접 개선사항을 테스트해보고 제안하는 자기주도성입니다. 나트리스는 작은 조직 특성상 빠른 시간 안에 산출물을 만들어내는 것이 중요했지만, 그만큼 많은 산출물이 쌓이면서 문서 정합성과 기획 의도 전달에 문제가 생겼습니다. 저는 이를 협업 구조의 문제로 보고, 흩어져 있던 산출물을 단일 GitHub 프로젝트로 통합해 AI 기반 검증으로 정합성을 자동으로 확인할 수 있는 구조를 만들었고, 화면 설계서는 기획자가 직접 제작한 프로토타입으로 대체해 이해관계자가 같은 결과물을 상상할 수 있도록 했습니다. 단순한 제안에 그치지 않고 직접 파일럿을 통해 결과물을 만들어 보였으며, 제작 문서 작성 기간을 1주일에서 3일로 단축했고, 해당 워크플로우는 조직에 정식 프로세스로 채택되어 운영되고 있습니다.
