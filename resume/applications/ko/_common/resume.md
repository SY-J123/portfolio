# Korea Common Resume Base

Last updated: 2026-04-06

## 기본 정보

장시영  
Product Manager  
jsyoung456@gmail.com  
010-3224-9483  
Seoul, Korea  
[Portfolio](https://portfolio-bn1.pages.dev/kr/) / [Velog](https://velog.io/@bryant_/posts)

## 한 줄 소개

운영과 서비스 기획을 모두 경험하며, 문제를 구조적으로 정의하고 협업 가능한 기준으로 구체화해온 Product Manager입니다.

## 핵심 역량

- 기능 기획 및 요구사항 정의
- UX 흐름 설계 및 정보 구조 정리
- AX 기반 협업 및 업무 효율 개선
- 운영 지표 모니터링 및 문제 대응
- 해외 지사 및 파트너 커뮤니케이션

## 경력

### Natris / LULU.AI
Product Manager | 2024.04 - Present

- B2C 모바일 웹 포커 게임의 기능 기획과 UX 설계 수행
- 기능 목적, 완료 기준, 예외 흐름까지 포함한 요구사항 정의
- 사용 맥락을 반영한 UX 정책과 화면 흐름 기준 정리
- AX 기반 협업 구조 개선 주도
- 성과: 이해관계자 투입 3명 -> 1명, 보고 및 검토 리드타임 1주 -> 3일 단축

### NSUSLAB Korea
Operations | 2023.08 - 2024.04

- GGPoker 토너먼트 운영 모니터링 및 운영 지표 점검
- 해외 지사 및 파트너와 이벤트 스케줄, 포맷, 운영 요청 사항 조율
- 전년도 손실이 컸던 운영 건 재점검 및 운영안 개선
- 성과: 손실 폭 절반 수준 감소

### Me2on
Operations | 2023.08 - 2024.08

- Fullhouse Casino 운영 지원 및 이슈 대응
- 해외 지사 커뮤니케이션과 운영 진행 상황 공유

## 주요 프로젝트

### AX 기반 문서 협업 프로세스 개선

- 문서 중심 검토 구조를 프로토타입 병행 방식으로 재설계
- Claude Code와 바이브 코딩을 활용해 검토와 수정 비용 절감
- 성과: 초기 검토 참여 인원 3명 -> 1명, 리드타임 1주 -> 3일 단축

### 운영 손실 대응

- 전년도 손실 운영 건의 상금 구조, 지표, 스케줄 재점검
- 더 적합한 운영안과 상금 규모 제안
- 성과: 손실 폭 절반 수준 감소

## 학력

- 방송통신대학교 데이터·통계학과 | 2023.03 - Present
- University of South Dakota Nursing | 2015.09 - 2016.08

## 자격

- SQLD
- ADsP
